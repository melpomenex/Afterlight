defmodule Afterlight.Accounts.Import do
  ...
  defp do_import(players, hash, source_path) do
    ...
    Repo.transaction(fn ->
      ...
      imported =
        Repo.one!(from p in "players", select: count(p.id))

      imported_coins =
        Repo.one!(from p in "players", select: coalesce(sum(p.coins), 0))

      imported_xp =
        Repo.one!(from p in "players", select: coalesce(sum(p.xp), 0))
