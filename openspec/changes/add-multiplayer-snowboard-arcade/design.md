...
`initialize()` remains synchronous: return cabinet/update/accept/dispose hooks immediately. `prepare()` is an internal async operation started by E; hold a token `{placeGeneration, attempt}` and disposed flag. Build the scene hidden, accept resources only for current token, and release only that attempt’s resource handles on late completion; do not dispose resources retained by another cache owner. Load first, then request play admission while still near cabinet; show cancellable loading state. If admission fails, keep a bounded cache and remain in world. A successful join starts lobby view and sends course readiness. A reconnect uses its existing server-reserved slot rather than fresh admission. Route E/button through an optional `instance.beginParticipation()` in the activity runtime before current `participation.interact()`; other types retain immediate join. Loading counts as a cancellable local occupied state for E/Escape and clears held world input. After network recovery, wait for accepted room membership, obtain authoritative resume acceptance/baseline and a fresh lease, then reacquire the view. Never replay pre-disconnect inputs.
...
- Save camera preference, first-person yaw/pitch, HUD state and prior presentation. Suspend world locomotion, world-only animation/audio and hidden cabinet visual work; continue net event processing, chat and theater synchronization. Hide theater DOM homography overlay without pausing shared media. Retain social world/Kiln in memory and authoritative social-room membership. Move leased race updates outside the current social `!paused` block: dialog/chat neutralizes controls but race presentation and authority processing continue. Suspension must not suppress the active activity update. Ambient suppression uses a temporary gain lease, never persisted mute/volume changes or chat/voice/theater interruption.
...
Session lookup identity becomes owner-derived `{canonicalRoomKey, roomEpoch, activityId}` for snowboard; `canonicalRoomKey` is `RoomKey.from_room(ownerRoom).room_key`, the existing canonical string derived from region, district and instance; this is not a new tuple API. Keep the real public wire `roomId` (`theater`) in existing envelopes; resolve manifest by district ID. Never construct the canonical identity from a client string. A random `sessionId` identifies the live process; a fresh `matchId` identifies each locked countdown. World instances with the same cabinet type cannot collide. Current public rooms only support instance `main`; non-main production travel is not added. Test identity isolation using two test owner instances, including owner epoch reuse/failover.
...
| `activity_join` offer acceptance | `{...F,requestId,role:'play',action:'accept_offer',offerId}`; use the queue control lease, validate current offer and proximity, rotate to a play lease. Default join action is absent; no other action accepted. |
| `activity_leave` pending cancel | `{requestId,activityId,cancelRequestId}`; only cancels the same connection's still-pending join attempt or its resulting unchanged lease. Keep a bounded 30s tombstone; cannot cancel another attempt/newer session. Local load-before-join cancel needs no command. |
...
Join acceptance extends current private `{result:'seated',slot,role,lease,leaseId,sessionId,revision,status,requestId}` with roomEpoch, matchId, serverNow and full baseline snapshot. Every correlated private ready/join/resnapshot acknowledgment includes requestId, serverNow and the same room/session identity for clock estimation. Initial lobby has a nonempty matchId; every locked countdown rotates it. Both `lease` and legacy `leaseId` aliases may be returned for compatibility, but only one canonical value. Watch/queue acknowledgement issues a scoped control lease if their leave/offer actions need it, never a play input lease.

Snapshot `activity_state` carries H plus `{status,matchId,snapshotSeq,serverTick,startAt,courseId,courseVersion,courseHash,deadlineAt,state:{players:[{playerId,nickname,accent,slot,connected,loaded,ready,status}],sim:{riders:[{playerId,s,u,v,vu,y,vy,grounded,jumpCharge,recoveryTicks,nextCheckpoint,splitsMs,finishMs,finishKey,dnfReason,resetSeq}],tick},lastAcceptedSeqs:{playerId:seq},queueLength,spectatorCount,result:null|latestResult}}`. Private reconciliation attachment is `{self:{appliedSeq,heldControls,serverTick}}`, never another rider’s input history. `appliedSeq` is updated only by a completed simulation step; `lastAcceptedSeqs` remains diagnostic and cannot acknowledge prediction replay. Fields unavailable before race are null/empty; nickname/accent are server-sanitized existing profile values. No raw input history or tokens broadcast. Max eight riders, eight splits each, bounded names, no full course in frames.
...
Public cabinet summary is `activity_state` with H plus `{audience:'summary',status,matchId,summary:{riderCount,readyCount,capacity:8,progress:[{playerId,nickname,nextCheckpoint,normalizedProgress,status}],result:null|latestResult}}`. Participant snapshots use `audience:'participants'`; route summaries to cabinet display only, never participation/reconciliation, and full snapshots to both. Summary progress is max eight riders.
...
30Hz local input/heartbeat (send neutral promptly), 30Hz server fixed steps, 20Hz full participant snapshots, immediate lifecycle events. Rendering stays display-rate. Existing 2KiB entire command cap, documented 60Hz input with burst10 (actual Phoenix guard currently uses 70/sec fixed windows; preserve that outer ceiling and enforce snowboard’s 30Hz target), 5Hz controls, resnapshot one/5s, 32KiB full snapshot and ≤20 snapshots/s are ceilings. Re-encode/check final snapshot; never merely trim events and assume it fits. Initial target rider snapshot ≤6KiB.
...
Client predictor runs the same fixed 30Hz rules with at most four local catch-up steps; keep up to two seconds/60 control steps. Include private appliedSeq/heldControls as D7 specifies. Record prediction steps separately from messages (load and neutral share the sequence stream but are not physics steps). On authoritative snapshot reset predicted simulation to server state/tick and heldControls, discard steps already represented by that tick, and replay remaining buffered per-tick samples with controls newer than appliedSeq layered over the canonical held state. Do not discard controls merely because lastAcceptedSeqs says they were received; they may await the next server tick. Seq is not a client time authority. Smooth visual correction ≤0.5m over 100ms, hard-reset >3m or any mismatch in checkpoint/recovery/grounded terminal state; intermediate errors converge over 150ms. Predicted effects are cosmetic; authoritative gate/finish events own HUD outcome. Freeze prediction after 250ms without snapshots and display reconnecting after 1s; don't continue inventing kilometers offline.
...
Lazy import does not pull Next.js/React/SSX engine into Afterlight. Cache one course resources bundle across rematches/short re-entry, in an application-owned reference-counted cache surviving activity disposal; evict shared resources after 60s unused, memory pressure or application teardown. Activity disposal immediately destroys its scene instances, listeners and audio and releases cache handles; it does not evict another live owner’s resources. No unconditional mountain prefetch at startup; optional proximity idle prefetch respects data-saver and never creates scene/AudioContext before need. Loading errors show Retry/Exit; ready disabled until all required resources/course hash match. Primitive cabinet fallback does not block play if the optional GLB fails.
