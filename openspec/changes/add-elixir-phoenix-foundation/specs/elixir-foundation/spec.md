...
The application supervision tree SHALL start exactly the Repo, a PubSub, telemetry, and the web Endpoint. It SHALL NOT start room processes, presence or membership structures, domain contexts, or any scheduler of game behavior, and the Endpoint SHALL NOT accept game protocol traffic. The Node server SHALL remain the sole authoritative server for the game.

#### Scenario: supervision tree starts and stays minimal

- **WHEN** the application starts in dev
- **THEN** the Repo, PubSub, telemetry, and Endpoint are running
- **AND** no room, presence, membership, or game-domain process exists
