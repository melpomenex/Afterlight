...
**10. Cinema view: seated watching becomes a first-class mode (user-requested addition).**
Sitting down transitions into a "cinema view": the video element leaves the projected in-world quad and reflows into a large 16:9 stage, the town chat panel docks full-height beside it, and the game HUD hides under a `body.theater-watching` class. A watch bar (now-playing title, Screen controls, "Stand up") plus Escape returns seamlessly to the isometric world at the exact seat — the player never moves, the engine never reloads, and sync/drift checks keep running (only the homography write is suspended while watching). Chat integration is CSS-only against `#chat-panel`/`#chat-toggle` (owned by the chat change): dock via scoped overrides, best-effort expand if collapsed, and degrade gracefully if the panel is absent.
*Why:* the homography screen is charming for the isometric view but too small to actually watch; DOM-based playback makes a big-screen mode nearly free, and the shared engine means switching presentation never interrupts the room. *Alternatives rejected:* iframe fullscreen API (fights the browser, loses chat beside the video); a separate /watch route (leaves the world, breaks co-presence feel).

**11. The Orpheum is the world spawn, in cinema view (user decision).**
The default initial room (no `?room=` override) is the theater, and `setRoom('theater')` engages cinema view on every entry — so players wake up in the lounge watching the shared screen with chat beside it. Movement keys, walk clicks, Escape, and the watch bar all step out of the view in place (watch-without-sitting is a pure presentation layer; the actor stands in the aisles the whole time). Sitting refines the same view with a "Stand up" action via the `onStandUpRequest` hook, and the watch bar labels itself accordingly (`setSeated`). Deep links (`?room=market`, `?room=garden`, any district id) keep working for anyone who wants the old landing.
*Why:* the product is a hangout first; spawning straight into the watching experience removes a navigation step and guarantees the screen has an audience. *Alternatives rejected:* cinema layout at spawn in other districts (no local screen, room state divorced from the view); spawn unchanged with theater-entry auto-cinema only (kept as behavior, but not the landing).
