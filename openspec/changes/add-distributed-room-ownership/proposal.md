...
- **Docs**: `runtime.md` §Room authority and partitioning linked as the design source; `migration.md` §4 gate recorded; new topology documented when the second node actually lands; `protocol-catalog.md` §6 "Declared tightenings" rows for the `epoch` field on room frames, the client epoch-discard rule, and instance switching being user-visible (entrance resets, emote-cooldown resets on instance change), each with client-impact analysis; the catalog's message tables and §5 client contract annotated with the `epoch` field.
