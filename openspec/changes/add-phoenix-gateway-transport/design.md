...
*Deployment assumption (review):* the one-live-transport-per-verified-identity invariant is enforced by a node-local ETS registry. P2 and P3 therefore assume a SINGLE gateway node; running two Phoenix gateway nodes behind a balancer would let each accept a transport for the same identity (Node would hold two sessions for one guestId — exactly the ghost this change prevents). Making the session registry fenced across nodes is a P9+ deliverable and MUST be named in `add-distributed-room-ownership` before a second gateway node runs.
