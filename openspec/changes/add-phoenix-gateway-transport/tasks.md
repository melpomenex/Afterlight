...
- [x] 6.4 Run the protocol-catalog §5 regression list on the Phoenix path: self-echo filtering via guestId continuity, duplicate-handler ordering, airborne flag edge (receiver without the flag stays grounded), `error` message consumers (theaterScreen bare `error` handling), duplicate `join_room` no-op for presence. *(Scripted two Phoenix sessions via `scripts/verify-gateway-transport.mjs`, 2026-09-07. Not two browsers.)*
...
- [x] 7.2 Document the rollback procedure (flip `VITE_TRANSPORT`/`VITE_WS_URL` back to Node; no durable state moved) in the deployment README notes, and rehearse it once in dev as part of 6.3 evidence. *(Rehearsal: secret-less Node `hello`→`welcome` at the end of `scripts/verify-gateway-transport.mjs`. Human two-browser rollback UI not run.)*
