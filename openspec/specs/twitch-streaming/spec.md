...
- **WHEN** the Twitch player API does not report ready within the client's timeout and the retry also fails
- **THEN** the client replaces the player with the official non-interactive embed for the same content, without advancing the bill for the room and without showing a room-wide failure

#### Scenario: Sound turns on over a non-interactive embed
- **WHEN** the master sound gate turns on while a Twitch clip or fallback embed is playing
- **THEN** the client reloads the embed without the mute flag so the content is audible, and turning the gate off re-mutes it the same way
