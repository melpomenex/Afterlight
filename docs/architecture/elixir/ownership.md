...
|---|---|---|---|---|---|---|---|---|---|---|
| 1 | Accounts / guest identity | `server/index.js` hello handler; guestId is client-generated and trusted | `game-state.json` `players` | `Afterlight.Accounts` (Ash) | `players` (read-only shadow until P6), `guest_sessions` tables | P4 session authority (signed socket identity + durable `guest_sessions`; player data stays Node-authored until the P6 economy-group flip) | `src/net/client.js` storage keys | Channels `connect` + hello still relayed to Node | Signed guest token → durable `GuestSession`; `hello`/`set_nickname` stay Node | P4 rollback: revert to P2 transient session registry; Node untouched. Player reverse export is owned by P6. |
| 2 | Nicknames | `shared/identity.js` sanitize/dedup in `server/index.js` | `players[].nickname` | `Afterlight.Accounts` | `players.nickname` (constraint created in P4, enforced from P6) | P6 (sanitize/dedup ports land in P4; routing stays Node until the economy-group flip) | `src/net/client.js`, `src/render/avatars.js` (palette) | Channels | Ash action `set_nickname` (P6); P4 still relays `set_nickname` to Node | Same as #1 |
| 3 | World movement | `server/world.js` (session pose, 10 Hz dirty-room flush) | none (ephemeral) | `Afterlight.World` room owner processes | none (transient) | P3 (transport relayed via the P2 gateway until then) | `src/main.js` + `src/render/avatars.js` | Channels room topic, coalesced snapshots | none (not durable) | n/a — transport-only; reconnect = fresh snapshot |
| 4 | Room membership | `server/world.js` rooms Map | none (ephemeral) | `Afterlight.World` (Registry + DynamicSupervisor + Presence for coarse membership) | none (Presence only) | P3 | client joinRoom/desiredRoom replay | Channels topic per room | n/a | n/a |
...
| 16 | Chat (game channels, DMs, history ring) | `server/chat.js` | none (in-memory 100-msg ring) | `Afterlight.Social` | optional `chat_messages` (history persistence is additive; default stays ephemeral) | `add-social-chat-relay` (P7 window; relayed to Node until then) | `src/ui/chatPanel.js` | Channels / `chat:*` topics | none required (relay) | n/a; history loss on restart is current behavior |
...
| 22 | Weather | `server/index.js` interval | none | `Afterlight.World` room runtime | none (deterministic rotation) | P6 (with gardens/economy; relayed from Node until that flip) | HUD | broadcast | n/a | n/a |
