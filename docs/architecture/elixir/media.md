...
Grant ordering invariant: targeted `torrent_grant` frames are pushed to each occupant strictly before the announcing `theater_state` snapshot, keeping the shared state token-free and preventing unsigned first requests. Refusals emit bounded diagnostic categories (`grant_missing`, `grant_expired`, `grant_invalid`, `stream_404`, `stream_416`, `stream_503`, `stream_504`, `stream_stalled`) with secrets and tokens redacted. Multi-file torrents are added with `deselect: true` so only chosen files/ranges transfer.
