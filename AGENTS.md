...
Product identity: the featured shared places — The Orpheum, The Rain Court, and accepted destinations after them — are what the game is. The market-garden/economy/restoration domain and the "Gardener" identity were **retired in full** by `remove-gardening-domain`: there is no cultivation, gathering, crafting, market, coins, XP or inventory anywhere in the client, the wire protocol, the Node sidecar or the Elixir/Phoenix stack. The Market Court remains as a quiet scenery-only square, the legacy districts keep their field notes and restoration landmarks, and the protected `data/*.json` snapshots stay read-only.
...
| `src/arcade/` | The canonical arcade cabinet system: ONE GLB (`public/arcade/cabinet/afterlight_arcade_cabinet.glb`) instanced per machine and reskinned through data — `cabinet.js` (load-once template, per-cabinet material clones, screen composite, LED/control application, INT_* markers, primitive fallback with hot-swap), `skins.js` (pure normalization/validation/aspect-fit), `artwork.js` (procedural per-motif artwork, motifs incl. `kart` and `downhill`). Game skins live in each activity definition's `cabinet` block in `shared/placeDefinitions.js`; the Orpheum row is Pong, Rain Runner, Downhill Mayhem, Kart Royale and Summit Run (Signal Lost dormant-but-present, re-placeable by a manifest edit). Kart Royale is the full external game hosted from `games/kart-royale` through the activity view lease's `present` hook; Downhill Mayhem is the hosted `games/downhill-mayhem` racer (next row). Authoring guide: `docs/arcade.md`; contracts: `tests/arcade-cabinet.test.js`. |
| Downhill Mayhem integration | Server-authoritative six-rider mountain-bike activity. Client: `src/activities/downhill-mayhem.js` (static bystander + lazy controller import) and `src/activities/downhill/{controller,scene,prediction,interpolation,clock,hud,audio}.js` (lazy; prediction/interpolation/clock generalized from Summit Run). Game + host runtime: `games/downhill-mayhem/src/{game,host,standalone}.js` (one shared `three@^0.185.1`; borrowed renderer/view lease, no second canvas/RAF/socket). Shared pure core: `shared/downhill/{rules,ai,course,courseDocument,courseHash}.js` + committed `shared/downhill/courses/*.json`; Elixir authority: `server_elixir/lib/afterlight/activities/downhill_mayhem.ex` plus `.../downhill_mayhem/` (course, AI, session policy, presentation) with the flag `AFTERLIGHT_DOWNHILL_MAYHEM_ENABLED`. Guides: `docs/downhill-mayhem.md`, `docs/arcade.md`; gate: `scripts/downhill-mayhem-gate-browser.mjs`. |
...
| Theater environments | Six authored worlds around the ONE Orpheum (coastal, rainforest, alpine, desert, redwood, cloud), selected through the room-authoritative atmosphere preset. Manifest: `shared/theaterEnvironments.js` (pure, 18 `env-*` variants); builders + runtime: `src/environments/` (`registry.js`, `runtime.js`, `quality.js` tier budgets incl. `openWaterSegments`, per-world builders, `lib/`); UI: `src/ui/worldSelector.js` behind the footer **◈ World** control; server: the projection's per-room `atmosphere.presets` allow-list + `atmosphere_set` (Router/GameChannel/`Atmosphere.set_preset`/RoomServer broadcast). Guide: `docs/theater-environments.md`; tests: `tests/world-selector.test.js`, `tests/environment-audio.test.js`, `server_elixir/test/afterlight/world/atmosphere_test.exs`. |
| `src/ui/profileModal.js` | Nickname-only player profile (`UIManager`): Visitor Pass identity, nickname editing, header nickname write. Replaced the old market/satchel/machine UI with the gardening retirement. |
...
| Floating mini-game media | One playback session, four presentations: world quad, cinema stage, floating player and hidden. Entry is a generation/attempt-fenced lease (`src/activities/mediaPresentation.js`, acquired by `activityRuntime.enterActivity` before lazy load/join); the theater UI keeps the one surface and swaps classes only. Pure policy/layout live in `src/ui/mediaPresentationState.js`, `src/ui/floatingMediaLayout.js`, `src/ui/floatingMediaReservations.js`; chrome/bindings in `src/ui/floatingMedia.js` (icon controls with accessible names/tooltips, hide/restore, enlarge/reduce, drag + handle menu, Back to game, honest degraded-provider notice); `src/ui/appFullscreen.js` (app-root fullscreen) and `src/ui/pointerLockBridge.js` (future explicit-gesture contract). Activity modules opt out or declare HUD reservations via `mediaPolicy` in `src/activities/registry.js`. Mute/unmute never sends a room action; rollback is `?floating-media=off` / `localStorage afterlight-floating-media = off`. Gate: `scripts/floating-media-gate-browser.mjs`. |
...
Stack: vanilla JavaScript ES modules, Three.js, Vite, DOM/CSS HUD, Web Audio. The supported multiplayer stack is **Phoenix gateway + Node specialty sidecar** (`npm run dev:stack`): Phoenix owns transport, world presence (dev flip), chat relay (dev flip), and signed identity; Node retains torrent/IRC HTTP, theater uploads, and transitional WS relay for the remaining domains (see `docs/architecture/elixir/ownership.md` §5). Legacy Node-only transport (`npm run dev` + `npm run server`) is deprecated (removal 2026-12-01).
...
1. Resolves the requested ID (valid deep link wins; absent link stays The Orpheum; unknown IDs — including the retired `garden` — visibly fall back to Theater, never market under a mismatched wire ID).
...
- Shift runs. E interacts. C cycles four views (three isometric angles, then first person). T opens Places.
...
- **Twitch is embed-only with per-type transport.** `shared/theaterModel.js` classifies Twitch share links (`twitch.tv/<channel>`, `twitch.tv/videos/<id>`, `clips.twitch.tv/<slug>`, `twitch.tv/<channel>/clip/<slug>`) as `kind: 'twitch'` with `twitchType`/`twitchId`, rejects non-content pages, and `isSeekSupported(item)` marks live HLS, Twitch channels, and Twitch clips non-seekable (the Elixir `Afterlight.Theater.Reducer` mirrors this; parity lives in `tests/fixtures/parity/theater-model.json`). Channels and VODs play through the official interactive player (`player.twitch.tv/js/embed/v1.js`); clips always use the non-interactive `clips.twitch.tv/embed` iframe, as does any SDK load failure. Twitch exposes no documented fatal-source event, so `isSourceFatalFailure` keeps Twitch trouble local: blocked autoplay shows the gesture badge, OFFLINE keeps the item on the bill and shows the offline card, and a never-ready player rebuilds once then stays local. Clips have no end event, so the pure `shouldAdvanceTwitchClip` guard (`TWITCH_CLIP_GUARD_SEC`, 75 s) advances an unattended clip via the once-per-item `ended` report; the booth disables clip pause/seek and keeps Skip. Every embed's `parent` comes from `window.location.hostname`, so localhost/preview/production hostnames work; nested-iframe ancestors are not discoverable and remain a documented limitation. Overlay sizing floors Twitch items at 400×300 CSS px (Twitch's minimum embed size) and the homography maps the larger element onto the same quad.
...
The Orpheum's surroundings are a shared room property selected through the
room-authoritative atmosphere preset — never a private display setting, and
never a reason to rebuild or move the theater. `shared/theaterEnvironments.js`
is the pure manifest (six worlds × three variants = 18 `env-*` presets);
`src/environments/` resolves and swaps the cached surrounding world;
`src/ui/worldSelector.js` is the **◈ World** dialog; `updateEnvironmentAudio()`
hands the active variant's `audio` row to the shared ambience bus.

Selection path: the dialog applies the preset locally at once (offline preview)
and sends `atmosphere_set { preset }`; the projection's per-room
`atmosphere.presets` allow-list is the server authority. `GameChannel` routes
and rate-limits the op (Phoenix-only), `Atmosphere.set_preset/4` validates the
id and performs a full replacement (revision bump, event-window rebuild,
`started_at` reset; seed/epoch unchanged), and the RoomServer broadcasts one
`atmosphere_state` to every member. Any live member may choose; the accepted
snapshot outranks any stale local pick. Keep the water-shader options opt-in
with backward-compatible defaults and stay inside the `quality.js` tier
ceilings (the open sea has `openWaterSegments`). Full guide:
`docs/theater-environments.md`.
...
```js
{
  ...
  exploration: {
    current: 'canal',
    visited: ['court', 'canal', 'station'],
    completed: ['canal'],  // Completed non-courtyard district IDs
  },
}
```
...
The HUD is identity-first (the gardening retirement). The header shows place, weather and clock; the companion panel shows the nickname profile (`src/ui/profileModal.js`, nickname editing only) and the network indicator; the footer carries Places, Records, Tournament, Emotes, Call, Camera, Sound and Settings. There is no tool belt, no coin/reputation/XP pills, no satchel/inventory or market control, and no legacy-HUD preference — `1`–`6`, `I` and `M` are no longer bound to game surfaces (the emote wheel owns number keys while it is open). Keep every HUD DOM id referenced by retained code stable; new UI must follow the existing translucent-panel language rather than reintroducing legacy farming controls.
