...
/// Spawn rows: id mask (SORTED_IDS) + archetype u16 + variant u16 +
/// stringRef u32 + x/y/z/yaw f32, all COLUMNAR (§3: "payloads are columnar
/// within a section"; the spawn field set follows the §3 string-table note).
...
pub fn apply_to_store(st: &mut Store, frame: &[u8]) -> u32 {
    ...
    let hflags = frame[6];
    if hflags & 0xFE != 0 {
        return E_BAD_HEADER; // bits 1.. reserved
    }
    ...
    while p < len {
        ...
        match sid {
            SEC_SPAWN | SEC_DESPAWN => {
                if enc != ENC_SORTED {
                    return E_BAD_ENCODING; // mask-only / id-carrying sections need an id list
                }
            }
            SEC_STRING_TABLE => {
                if !has_string_table {
        ...
        if sid == SEC_STRING_TABLE {
            ...
            let expected = mask + cols_per_row(sid) * count as u64;
            if plen != expected {
                return E_PAYLOAD_LEN;
            }
            // enums + id ordering are checked before any mutation
            let cols = start + (mask as usize);
            match (sid, enc) {
                (SEC_FLAGS, _) => {
...
fn apply_spawn(st: &mut Store, b: &[u8], s: &Section, string_count: u32) -> u32 {
    ...
    let ids = s.start;
    let arch = ids + 4 * c;
    let variant = arch + 2 * c;
    let sref = variant + 2 * c;
    let x = sref + 4 * c;
    for i in 0..c {
        let id = ru32(b, ids + i * 4);
        if st.contains_id(id) {
            return E_ID_EXISTS;
        }
        let sr = ru32(b, sref + i * 4);
        if string_count > 0 {
            if sr >= string_count {
                return E_BAD_STRING_TABLE;
            }
        ...
        let u = slot as usize;
        st.archetype[u] = ru16(b, arch + i * 2) as u16;
        st.variant[u] = ru16(b, variant + i * 2) as u16;
        st.string_ref[u] = sr;
        // columns are contiguous per field (§3): x then y then z then yaw
        st.x[u] = rf32(b, x + i * 4);
        st.y[u] = rf32(b, x + 4 * c + i * 4);
        st.z[u] = rf32(b, x + 8 * c + i * 4);
        st.yaw[u] = rf32(b, x + 12 * c + i * 4);
