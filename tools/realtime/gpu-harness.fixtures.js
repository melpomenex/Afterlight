// Deterministic population fixtures for the GPU harness page
// (tools/realtime/gpu-harness.html, openspec change add-realtime-gpu-rendering
// task 3.2).
//
// This is a faithful BROWSER-SIDE MIRROR of the world generator in
// benchmarks/realtime/lib/fixtures.mjs (rng, WORLD_BOUNDS, makeWorld,
// stepWorld) — same seeds produce the same worlds. It is mirrored instead of
// imported because plain static file servers (python3 -m http.server) do not
// reliably serve .mjs with a JavaScript MIME type, and the harness page is
// required to run without a build step. If fixtures.mjs changes, update this
// mirror in lockstep; the harness report records the fixture source so a
// stale mirror cannot pass unnoticed.
//
// Semantics (docs/architecture/realtime/contract.md §2/§8): a "world" is the
// server-side authoritative snapshot — SoA columns over dense local slots
// 0..n-1 with sparse gap-y u32 server ids; stepWorld advances one 100 ms tick
// and returns the sorted changed-id list.

export const WORLD_BOUNDS = { minX: -11.3, maxX: 11.3, minZ: -9.5, maxZ: 10.3, maxY: 0.72 };

export const FIXTURE_SOURCE = 'mirror of benchmarks/realtime/lib/fixtures.mjs (rng/WORLD_BOUNDS/makeWorld/stepWorld)';

// Seeded LCG (repo convention: deterministic procedural randomness).
...
// Deterministic per-tick seed so a harness run is reproducible end to end
// (world seed + tick index → same changed set every run).
export function tickSeedFor(worldSeed, tickIndex) {
  return (worldSeed ^ Math.imul(tickIndex + 1, 0x9E3779B9)) >>> 0;
}
